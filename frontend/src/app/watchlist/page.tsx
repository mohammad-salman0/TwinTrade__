...
function RSIBar({ rsi }: { rsi: number | null }) {
  ...
  const color =
    rsi < 30 ? "var(--up)" : rsi > 70 ? "var(--down)" : "var(--warn)";
  return (
...
export default function WatchlistPage() {
  ...
  const fetchAnalysis = useCallback(
    async (symbol: string): Promise<StockAnalysis> => {
      try {
  ...
  const loadAnalysis = useCallback(
    async (list: WatchlistStock[]) => {
      if (!list.length) return;
      const results = await Promise.all(
        list.map((s) => fetchAnalysis(s.symbol)),
      );
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          ...
          {loading ? (
            ...
            >
              ...
              >
                Browse market stocks and add your favorites here.
              </p>
              <Link href="/stocks">
                <button
            ...
            >
              ...
              >
                ...
                >
                  Monitored Stocks — Live Analysis
                </span>
                ...
                return (
                  <div
                    ...
                    style={{
                      display: "grid",
                      gridTemplateColumns:
                        "2fr 1.2fr 1fr 1.4fr 1.4fr 1.2fr 1fr",
                      padding: "16px 22px",
                      borderBottom:
                        i < stocks.length - 1
                          ...
                          {a.price && a.high52w && a.low52w && (
                            ...
                            >
                              <div
                                style={{
                                  height: "100%",
                                  width: `${Math.min(((a.price - a.low52w) / (a.high52w - a.low52w)) * 100, 100)}%`,
                        ...
                        style={{
                          ...
                          color:
                            removing === stock.symbol
                              ? "var(--text-muted)"
                              : "var(--down)",
                          border: `1px solid ${removing === stock.symbol ? "var(--border)" : "var(--down-border)"}`,
                          opacity: removing === stock.symbol ? 0.6 : 1,
                          width: "100%",
                          justifyContent: "center",
                        }}
                      >
                        <Trash2 size={10} />
                        {removing === stock.symbol ? "..." : "Remove"}
                      </button>
                    </div>
                  </div>
                );
                ...
                {[
                  {
                    icon: Activity,
                    label: "RSI < 30",
                    desc: "Oversold — potential buy zone",
                    color: "var(--up)",
                  },
                  {
                    icon: Activity,
                    label: "RSI > 70",
                    desc: "Overbought — potential sell zone",
                    color: "var(--down)",
                  },
                  {
                    icon: BarChart2,
                    label: "SMA Bullish",
                    desc: "Price > SMA20 > SMA50 — uptrend",
                    color: "var(--up)",
                  },
                  {
                    icon: BarChart2,
                    label: "SMA Bearish",
                    desc: "Price < SMA20 < SMA50 — downtrend",
                    color: "var(--down)",
                  },
                  {
                    icon: TrendingUp,
                    label: "52W Range bar",
                    desc: "Position of current price within 52-week range",
                    color: "var(--accent-teal)",
                  },
                ].map((item) => {
