...
Runs continuously inside the same process as server.js. Every
60 seconds it:

  1. Reads the full symbol list from screeningService.js (the
     same CSV-backed 500-stock list stockController.js uses
     for pagination — NOT the separate/stale Stock MongoDB
     collection, which importStocks.js seeds from a different,
     much shorter hardcoded list)
...
const { getScreenedStocks } = require("./services/screeningService");
...
async function runOneCycle() {
  const screenedStocks = await getScreenedStocks();
  const symbols = screenedStocks.map((s) => s.symbol);

  if (!symbols.length) {
    console.warn(
      "Worker: no symbols found from screeningService, skipping cycle",
    );
    return;
  }
...
Unlike an earlier version of this file, this no longer needs
MongoDB at all — the symbol list comes from screeningService.js
(CSV-backed), not the database. server.js still connects to
Mongo for everything else the app needs, but this worker would
run fine even if that connection were slow to establish.
=====================================
*/

module.exports = function startPriceWorker() {
  console.log("Starting price refresh loop");
  refreshLoop();
};
...
// server.js already calls connectDB() before this is invoked,
// so this file assumes Mongo is already connected — it only
// owns the refresh loop, not the DB connection.
// =====================================
// */
